...
import { Row, Button, Col, Icon, Form, message,Alert,Tag } from 'antd';
import './index.scss';
    ...
    return request(`${doneApiPrefix}/dir/list`, {
      ...
      if (res.code === 200) {
        this.setState(
          ...
          () => {
            if (!isManual && !localStorage.getItem('tree_select')) this.getCaseList(1, '', '', '', []);
            
          },
        );
        if (!isManual &&  localStorage.getItem('tree_select')){
          
           setTimeout(() => {
            if(this.filetree)
              this.filetree.setTreeSelect();
           }, 100);
        }
      } else {
          ...
          <div className="site-drawer-render-in-current-wrapper">
            ...
            <Row className="">
              ...
              <Col xs={6} className="text-right">
                ...
                <Button
                  type="primary"
                  // style={{background: '#5A4B87' ,borderColor:'#5A4B87'}}
                  onClick={() => {
                    this.handleTask('add');
                    this.setState({
                      currCase: null,
