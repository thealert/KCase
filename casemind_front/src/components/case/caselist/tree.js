...
class FileTree extends React.Component {
  ...
  addNode = (key, data) => {
    ...
    const newTree = data.map(item => {
      ...
      if (key === item.id) {
        ...
        return { ...item, children: [newNode,...item.children] };
      }
            ...
            <span className="iconShow">
              <Dropdown
                overlay={
                  ...
                  >
                    {item.id !== 'root' && item.parentId!= 'root' ? (
  ...
  onDrop = info => {
    ...
    //console.log(dragNode.pos.split('-').length)
    // if(dragNode.pos.split('-').length<=3){
    //   message.error('二层级以上不许移动');
    //   return
    // }
  ...
  setTreeSelect = ()=>{
     ...
     var tree_select=localStorage.getItem('tree_select')
     if(tree_select){
      this.setState({expandedKeys:[tree_select]})
      this.setState({treeSelect:tree_select})
      this.addMenber([tree_select]);
     }
  }
  ...
  render() {
    const { treeSelect, expandedKeys, autoExpandParent, treeData,showTree } = this.state;
    return treeData?.length > 0 ? (
      <div>
      {/* <button style={{ position: 'absolute', left: 10, top: 5 }}>-</button> */}
      <Tooltip title="展开/折叠目录">
        {showTree ? 
          
          <Icon type="double-left" onClick={this.showhideTree} style={{ marginTop: '6px', color:"grey",fontSize:'18px',position: 'absolute',zIndex:9999, left: 10, top: 5 }}></Icon>
        : <Icon type="double-right" onClick={this.showhideTree} style={{ marginTop: '0px', color:"grey",fontSize:'14px',position: 'absolute',zIndex:9999, left: 10, top: 5 }}></Icon>
        
        }
      </Tooltip>
      { showTree && <ResizePanel
        direction="e"
        style={{  backgroundColor: "white",width: "230px",height:"calc(100vh)" }}
      >
                  ...
                  if (selectedKeys.length > 0) {
                    ...
                    if (this.state.treeSelect != newSelect) {
                      this.setState({ treeSelect: newSelect }, () => {
                        ...
                        localStorage.setItem('tree_select',newSelect)
