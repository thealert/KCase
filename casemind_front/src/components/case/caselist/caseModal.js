...
const initData = `{"root":{"data":{"id":"bv8nxhi3c800","created":`+new Date().getTime()+`,"text":"中心主题"},"children":[]},"template":"right","theme":"fresh-blue","version":"1.4.43","base":0}`;
...
class CaseModal extends React.Component {
  ...
  getCardTree = () => {
    request(`${this.props.doneApiPrefix}/dir/cardTree`, {
      ...
      let cardTree = res.data ? res.data.children : [];
  ...
  saveEditerData(values) {
    ...
    if (caseFile) {
      if (/(?:xmind|mm)$/i.test(caseFile.name)) {
      ...
      >
        ...
        <Form.Item {...formItemLayout} label="用例集名称：">
          {getFieldDecorator('case', {
            ...
            initialValue: data
              ? (operate == 'copy' && `副本 ${data.title}`) || data.title
              : '',
          ...
          {getFieldDecorator('bizId', {
            ...
            <TreeSelect
             ...
             filterTreeNode={(search, item) => {
              //console.log(item.props.dataRef.text)
              return item.props.dataRef.text.toLowerCase().indexOf(search.toLowerCase()) >= 0;
            }}
            >
              ...
              <div className="div-flex-child-2">
                <div>
                  ...
                  <span className="span-text span-text-light">
                    支持.xmind,.mm和excel文件，excel请按照<a href={caseTemplate} download="testcase-template.xlsx">
                      模板
                    </a>填写...
                  </span>
                </div>
