...
class MediaGroup extends Component {
  ...
  render() {
    ...
    const { showLink, showImage, showNote, noteStatus, drawerVisible, imageModle } = this.state;
      ...
      >
        ...
        >
          <Button type="link" size="small" 
            className={`big-icon ${theme == 'dark' && !disabled ? 'dark-theme-button' : ''}`}
        ...
        {toolbar.addFactor && (
          ...
          >
            ...
            <Button type="link" size="small" className={`big-icon ${theme == 'dark' && !disabled ? 'dark-theme-button' : ''}`}
             style={{ marginRight: 10 ,color: (theme=='dark'? (!disabled?'#d5d5d5':'grey') :'')}} >
