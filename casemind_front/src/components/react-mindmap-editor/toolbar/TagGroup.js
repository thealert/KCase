import React, { Component } from 'react'
import { Popconfirm, Tag, Input, Icon, message, Tooltip } from 'antd'
import { getUsedResource } from '../util'
import debounce from 'lodash/debounce'

class TagGroup extends Component {
  state = {
    value: '',
    selectedResource: [],
    newTags: [],
    inputVisible: false,
    inputValue: '',
    confirmVisible: false,
  }
  componentDidMount() {
    let { minder } = this.props
    minder.on('contentchange selectionchange', () => {
      const selectedResource = minder.queryCommandValue('Resource')
      this.setState({ selectedResource })
    })
  }
  handleClose = removedTag => {
    const newTags = this.state.newTags.filter(tag => tag !== removedTag)
    this.setState({ newTags })
  }
  handleOpenConfirm = () => {
    this.setState({ confirmVisible: true })
  }
  handleCancelConfirm = () => {
    this.setState({ confirmVisible: false })
  }

  handleConfirm = () => {
    this.setState({ confirmVisible: false })
    this.showInput()
  }
  showInput = () => {
    window.tagInput = true
    this.setState({ inputVisible: true }, () => this.input.focus())
  }
  handleInputChange = e => {
    this.setState({ inputValue: e.target.value })
  }
  handleInputConfirm = () => {
    const { inputValue } = this.state
    let { newTags } = this.state
    const { minder } = this.props
    let tags = getUsedResource(minder.getAllNode())
    if (inputValue && newTags.indexOf(inputValue) === -1 && tags.indexOf(inputValue) === -1) {
      newTags = [...newTags, inputValue]
    } else if (newTags.indexOf(inputValue) !== -1 || tags.indexOf(inputValue) !== -1) {
      message.warning('该标识已存在！')
    }
    this.setState(
      {
        newTags,
        inputVisible: false,
        inputValue: '',
      },
      () => {
        window.tagInput = false
      },
    )
  }
  saveInputRef = input => (this.input = input)
  onAdd = newResource => {
    const { minder } = this.props
    let resource = []
    const oldResource = minder.queryCommandValue('Resource')
    if (oldResource.indexOf(newResource) < 0) {
      resource = [...oldResource, newResource]
      minder.execCommand('Resource', resource)
    }
  }
  onRemove = oldResource => {
    const { minder } = this.props
    let resource = []
    const current = minder.queryCommandValue('Resource')
    if (current.indexOf(oldResource) > -1) {
      resource = current.filter(item => item !== oldResource && item !== null)
      if (resource.length === 0) {
        resource = null
      }
    }
    minder.execCommand('Resource', resource)
  }
  handleClick = (e, item) => {
    const { minder } = this.props
    let resource = []
    const oldResource = minder.queryCommandValue('Resource')
    console.log(oldResource)
    console.log(item)
    if (oldResource && oldResource.includes(item)) {
      //console.log("有标签")
      resource = oldResource.filter(curitem => curitem !== item && curitem !== null)
      if (resource.length === 0) {
        resource = null
      }
      minder.execCommand('Resource', resource)
    } else {
      //console.log("没有标签")
      resource = [...oldResource, item]
      minder.execCommand('Resource', resource)
    }

    // if (!this._delayedClick) {
    //   this._delayedClick = debounce(this.doClick, 500);
    // }
    // if (this.clickedOnce) {
    //   this._delayedClick.cancel();
    //   this.clickedOnce = false;
    //   this.onRemove(item);
    // } else {
    //   this._delayedClick(e);
    //   this.clickedOnce = true;
    //   this.onAdd(item);
    // }
  }
  doClick = () => {
    this.clickedOnce = undefined
  }
  renderTags = (itemList, closable) => {
    const { minder, isLock } = this.props
    let disabled = minder.getSelectedNodes().length === 0
    if (isLock) disabled = true
    return itemList.map(item => {
    
      const backgroundColor = minder.getResourceColor(item).toHEX()
      return (
        <Tag
          key={item}
          className={`resource-tag${disabled ? ' disabled' : ''}`}
          style={{ fontSize: '12px',marginTop:'5px' }}
          disabled={disabled}
          closable={closable}
          color={backgroundColor}
          onClick={e => (disabled ? null : this.handleClick(e, item))}
          onClose={() => this.handleClose(item)}
        >
          {item}
        </Tag>
      )
    })
  }
  render() {
    const { newTags, inputVisible, inputValue, confirmVisible } = this.state
    const { minder, tags, theme } = this.props
    let resourceList = getUsedResource(minder.getAllNode())
    let oldResource = resourceList.filter(
      item => newTags.indexOf(item) < 0 && tags.indexOf(item) < 0 && item !== 'undefined',
    )

    return (
      // <div className="nodes-actions" style={{ width:600 padding: '0px 4px' }}>
      <div className="nodes-actions" style={{ width:530,padding: '0px 4px' }}>
        <Tooltip
          // title="选中节点后，单击添加标签，双击移除标签。"
          title="选中节点后，单击添加移除标签。"
          getPopupContainer={triggerNode => triggerNode.parentNode}
        >
          <div style={{ display: 'inline-block' }}>
            {/* {tags && this.renderTags(tags, false)} */}
            {/* <span style={{ marginRight: 8 }}>步骤:</span> */}
            {/* <Tag style={{ color: 'black' }} color="white">标签:</Tag> */}

            {tags && this.renderTags(tags.slice(0, 3), false)}
            {/* <span style={{ marginRight: 8 }}>方向:</span> */}

            {/* <Tag style={{ color: 'black', marginTop: '5px' }} color="white">|</Tag> */}
            <span>|&nbsp;</span>
            {tags && this.renderTags(tags.slice(3, 7), false)}

            {/* <Tag style={{ color: 'black', marginTop: '5px' }} color="white">|</Tag> */}
            <span>|&nbsp;</span>
            {tags && this.renderTags(tags.slice(7, 12), false)}
            <span>|&nbsp;</span>
            {tags && this.renderTags(tags.slice(12, 16), false)}
            {/* <Tag style={{color: 'black'}} color="white">自定义:</Tag>
            {tags && this.renderTags(tags.slice(12), false)} */}

            {oldResource && this.renderTags(oldResource, false)}

            {newTags && this.renderTags(newTags, true)}

            {inputVisible && (
              <Input
                ref={this.saveInputRef}
                type="text"
                size="small"
                style={{ width: 78 }}
                value={inputValue}
                onChange={this.handleInputChange}
                onBlur={this.handleInputConfirm}
                onPressEnter={this.handleInputConfirm}
              />
            )}
            {!inputVisible && (
              <Tag
                onClick={this.handleOpenConfirm}
                style={{ color: theme=='dark'?'#d5d5d5':'' ,background: theme=='dark'?'#232323':'#fff', borderStyle: 'dashed' }}
              >
              <Icon type="plus" /> 新增
            </Tag>
        )}
          </div>
        </Tooltip>
        

        <Popconfirm
          title="非特殊情况不建议创建自定义标签，确定要执行这个操作吗？"
          visible={confirmVisible}
          onConfirm={this.handleConfirm}
          onCancel={this.handleCancelConfirm}
          okText="确定"
          cancelText="取消"
        />
      </div>
    )
  }
}
export default TagGroup
