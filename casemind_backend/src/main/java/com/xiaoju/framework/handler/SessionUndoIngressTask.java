...
public class SessionUndoIngressTask extends SessionIngressTask{
    PushMessage data;

    public SessionUndoIngressTask(WebSocketSession session, SessionRoomEntity room, ExecutorService executorEgressService, PushMessage data) {
    ...
    public void run() {
        ...
        try {
            ...
            //executorEgressService.submit(new SessionNotifyExcludeEgressTask("undo", data.getMessage(), session,roomseesions));
            executorEgressService.submit(new SessionNotifyEgressTask("undo", data.getMessage(), roomseesions));
