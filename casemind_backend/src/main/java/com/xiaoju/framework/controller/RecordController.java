...
import com.xiaoju.framework.entity.request.record.*;
...
public class RecordController {
    ...
    @PostMapping(value = "/editprview")
    public Response<?> editPrviewRecord(@RequestBody RecordPrviewReq req) {
        req.validate();
        try {

            recordService.editPrviewRecord(req);
            return Response.success();
        } catch (CaseServerException e) {
            throw new CaseServerException(e.getLocalizedMessage(), e.getStatus());
        } catch (Exception e) {
            LOGGER.error("[列表页面-更新prview record属性出错]入参={}, 原因={}", req.toString(), e.getMessage());
            e.printStackTrace();
            return Response.build(StatusCode.SERVER_BUSY_ERROR);
        }
    }
