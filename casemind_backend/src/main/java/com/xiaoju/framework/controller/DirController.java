...
public class DirController {
    ...
    public Response<?> getDirTree(@RequestParam @NotNull(message = "业务线id为空") Long productLineId,
        ...
        return Response.success(dirService.getAllCaseDir(dirService.getDirTree(productLineId, channel),productLineId));
    }

    @GetMapping(value = "/getbusinessnames")
    public Response<?> getBusinessNames(@RequestParam @NotNull(message = "业务线id为空") Long productLineId,
                                  @RequestParam @NotNull(message = "渠道为空") Integer channel) {
        return Response.success(dirService.getDirBusineseNames(dirService.getDirTree(productLineId, channel)));
    }
    ...
    public Response<?> deleteDir(@RequestBody DirDeleteReq request) {
        ...
        try {
            return Response.success(dirService.getAllCaseDir(dirService.delDir(request),request.getProductLineId()));
        } catch (CaseServerException e) {
            throw new CaseServerException(e.getLocalizedMessage(), e.getStatus());
        } catch (Exception e) {
