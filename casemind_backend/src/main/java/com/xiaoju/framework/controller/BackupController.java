...
public class BackupController {
    ...
    public Response<CaseBackupListResp> getBackupByCaseId(@RequestParam @NotNull(message = "用例id为空") Long caseId,
                                                           @RequestParam(required = false) String beginTime,
                                                           @RequestParam(required = false) String endTime,
                                                           @RequestParam(defaultValue = "1") Integer pageNum,
                                                           @RequestParam(defaultValue = "10") Integer pageSize
                                                        ) {
        return Response.success(caseBackupService.getBackupByCaseId(caseId, beginTime, endTime,pageNum,pageSize));
    }
