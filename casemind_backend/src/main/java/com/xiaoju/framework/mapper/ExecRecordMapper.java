...
import com.xiaoju.framework.entity.dto.RecordAndPrviewNumDto;
...
public interface ExecRecordMapper {
    ...
    List<ExecRecord> getCaptureListByParentCaseId(Long parentId);
    List<ExecRecord> getCaptureListByParentCaseIds(List<Long> caseIds);

    List<ExecRecord> getRecordsByCaseIds(List<Long> caseIds);
    /**
    ...
    List<RecordNumDto> getPrviewDoneNumByCaseIds(List<Long> caseIds);
    List<RecordAndPrviewNumDto> getRecordPrviewNumByCaseIds(List<Long> caseIds);
    List<RecordNumDto> getCaptureNumByCaseIds(List<Long> caseIds);
    /**
     * 脑图更新执行任务，与统计数据有关
     *
     * @param record 任务实体
