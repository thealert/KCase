...
import com.xiaoju.framework.entity.request.record.RecordPrviewReq;
...
import com.xiaoju.framework.handler.SessionRecordEntity;
import com.xiaoju.framework.handler.SessionRecordFactory;
...
public class RecordServiceImpl implements RecordService {
    ...
    public List<RecordListResp> getListByCaseId(Long caseId) {
        ...
        List<ExecRecord> CaptureList = recordMapper.getCaptureListByParentCaseId(caseId);
        for (ExecRecord record : execRecordList) {
            RecordListResp recordlistresp=buildList(record);
            recordlistresp.setCase_extype(testCase.getCase_extype());
            res.add(recordlistresp);
        }
        for (ExecRecord capture : CaptureList) {
            RecordListResp recordlistresp=buildList(capture);
            recordlistresp.setCase_extype(testCase.getCase_extype());
            recordlistresp.setRecord_type(2);
            res.add(recordlistresp);
        }
    ...
    public RecordGeneralInfoResp getGeneralInfo(Long recordId) {
        ...
        // 修复：传入 recordId 以便从 WebSocket 内存中获取最新数据
        JSONObject merged = getData(new MergeCaseDto(testCase.getId(), record.getChooseContent(), record.getCaseContent(), record.getEnv(), recordId));
...
//        }
        ...
        record.setReview_result(req.getReview_result());
        record.setEnv(req.getEnv());
        recordMapper.edit(record);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void editPrviewRecord(RecordPrviewReq req) {
        ExecRecord record = recordMapper.selectOne(req.getId());
        record.setReview_result(req.getReview_result());
        record.setExecutors(req.getUser_name());
    ...
    private RecordListResp buildList(ExecRecord record) {
        ...
        resp.setRecord_type(record.getRecord_type());
        resp.setReview_result(record.getReview_result());
        resp.setEnv(record.getEnv());
        ...
        resp.setModifyTime(record.getGmtModified().getTime());
    ...
    private ExecRecord buildExecRecord(RecordAddReq req, JSONObject merged) {
        ...
        record.setRecord_type(req.getRecordtype());
        record.setReview_result(0);
        record.setIsDelete(NOT_DELETE);
        record.setCaseContent(EMPTY_STR);
        record.setNoteContent(EMPTY_STR);
...
//    }
    ...
    public JSONObject getData(MergeCaseDto dto) {
        ...
        // 优先从 WebSocket 内存中获取最新数据
        if (dto.getRecordId() > 0L) {
            String roomId = String.valueOf(BitBaseUtil.mergeLong(dto.getRecordId(), dto.getCaseId()));
            SessionRecordEntity recordEntity = SessionRecordFactory.getRoomByRoomID(roomId);
            if (recordEntity != null) {
                websocketCaseContent = recordEntity.getCaseContent();
                LOGGER.info("从 WebSocket 内存中获取最新数据，roomId: " + roomId + ", base: " + 
                    JSON.parseObject(websocketCaseContent).getInteger("base"));
            }
        }
        ...
        // 如果 WebSocket 中有更新的数据，使用 WebSocket 的数据
        if (websocketCaseContent != null) {
            content = JSON.parseObject(websocketCaseContent);
        }
