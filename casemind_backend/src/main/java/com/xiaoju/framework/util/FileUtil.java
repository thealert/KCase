...
public class FileUtil {
    ...
    public static String fileUpload(String path, MultipartFile file) {
        ...
        File targetFile = buildUploadTarget(path, oldName);
        try {
            file.transferTo(targetFile);
            return buildRelativePath(targetFile);
        } catch (IOException e) {
            throw new CaseServerException("图片上传失败：" + e.getMessage(), StatusCode.FILE_IMPORT_ERROR);
        }
    }

    public static String fileUpload(String path, File sourceFile, String originalFilename) {
        File targetFile = buildUploadTarget(path, originalFilename);
        try {
    ...
    private static File buildUploadTarget(String path, String originalFilename) {
        ...
        if (!folder.exists() && !folder.mkdirs()) { // 如果文件夹不存在，则新建文件夹
            throw new CaseServerException("图片上传失败：无法创建目录 " + folder.getAbsolutePath(), StatusCode.FILE_IMPORT_ERROR);
        }
        if (!folder.isDirectory()) {
            throw new CaseServerException("图片上传失败：上传目录不可用 " + folder.getAbsolutePath(), StatusCode.FILE_IMPORT_ERROR);
        }
        // 对上传的文件重命名，避免文件重名
        String oldName = StringUtils.isEmpty(originalFilename) ? UUID.randomUUID().toString() : originalFilename;
        int suffixIndex = oldName.lastIndexOf(".");
        String fileSuffix = suffixIndex >= 0 ? oldName.substring(suffixIndex) : "";
        String newName = UUID.randomUUID().toString() + fileSuffix; // 生成新的随机的文件名字
        return new File(folder, newName);
    }

    private static String buildRelativePath(File targetFile) {
        String folderPath = targetFile.getParentFile().getName();
